...
function SAVEQUEST_ON_INIT(addon, frame)
  ...
  addon:RegisterMsg("FPS_UPDATE", "SAVEQUEST_REMOVE_NPC_NEARBY");
...
function SAVEQUEST_REMOVE_NPC_NEARBY(frame, msg, argStr, argNum)
    local pc = GetMyPCObject();
    local list, cnt = SelectObjectByFaction(pc, 300, 'Neutral');

    for i = 1, cnt do
        local handle = GetHandle(list[i]);

        if handle ~= nil then
            saqu:RemoveQuestNpc(handle);
        end
    end
end
