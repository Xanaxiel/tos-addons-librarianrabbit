...
function _LR_TRANSLATE(txt)
	if g._nodeListLang == 1 then
		if g._translate[txt] then
			return g._translate[txt];
		else
			return txt;
		end
	else
...
function _LR_LOAD_WAREHOUSE()
	g.warehouseList = acutil.loadJSON(g.settingPath..'warehouse.json',nil);
end
-- }
...
-- LR edit {
function _LR_LOAD_ALL_ITEMLIST()
...
--	tolua.cast(frame:GetChild('tab'), "ui::CTabControl"):SelectTab(0)
	tolua.cast(frame:GetChild('tab'), "ui::CTabControl"):SelectTab(2)
-- }

	frame:GetChild('saveBtn'):SetTextTooltip(_LR_TRANSLATE('現在のキャラのインベントリを保存する'))
		...
		for i,value in ipairs(nodeList) do
			...
			if nodeItemList and not g.setting.hideNode[i] then
				if value[1] == "Silver" then
					...
					parentCategory = tree:FindByCaption(value[g._nodeListLang]);
...
-- 		end
		...
		local ilist = itemlist[cid]

		if not ilist then
			if iswarehouse == false then
				ilist ,e = acutil.loadJSON(g.settingPath..cid..'.json',{})

				if not e then
					g.itemlist[cid] = ilist;
				end
			end
		end
...
function BARRACKITEMLIST_SHOW_SEARCH_ITEMS(frame, obj, argStr, argNum)
	if g._firstSearch == true then
		local frame = ui.GetFrame('barrackitemlist')
		local searchGbox = frame:GetChild('searchGbox')
		local editbox = tolua.cast(searchGbox:GetChild('searchEdit'), "ui::CEditControl");

		if editbox:GetText() == '' or not editbox:GetText() then
			_BARRACKITEMLIST_SHOW_SEARCH_ITEMS(frame, obj, argStr, argNum);
		else
			ui.MsgBox("The first search might be really slow and lock your client temporarily. Do you want to continue?", "_BARRACKITEMLIST_SHOW_SEARCH_ITEMS", "None");
		end
	else
		_BARRACKITEMLIST_SHOW_SEARCH_ITEMS(frame, obj, argStr, argNum);
	end
...
function _BARRACKITEMLIST_SHOW_SEARCH_ITEMS(frame, obj, argStr, argNum)
-- }
	local frame = ui.GetFrame('barrackitemlist')
	local searchGbox = frame:GetChild('searchGbox')
	local editbox = tolua.cast(searchGbox:GetChild('searchEdit'), "ui::CEditControl");
...
function BARRACKITEMLIST_CREATE_SETTINGMENU()
	...
	for i = 1 ,#g.nodeList do
		...
		checkbox:SetText('{s30}{#000000}'..g.nodeList[i][g._nodeListLang])
